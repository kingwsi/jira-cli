...
func main() {
	...
	flag.IntVar(&port, "port", 8899, "HTTP 服务监听端口")
	flag.IntVar(&port, "p", 8899, "HTTP 服务监听端口 (简写)")
	flag.StringVar(&host, "host", "0.0.0.0", "HTTP 服务监听地址 (0.0.0.0 允许局域网访问)")
	flag.StringVar(&host, "H", "0.0.0.0", "HTTP 服务监听地址 (简写)")
	flag.BoolVar(&openBrowser, "open", false, "后台启动服务并在浏览器中打开工作台")
	...
	if err := server.Launch(cfg); err != nil {
