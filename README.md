...
启动控制台会打印检查结果；发现新版本时，控制台展示版本号及简短更新说明，页面顶部栏展示版本号和红点，点击顶部提醒可直达更新设置。页面每 30 秒读取服务端缓存状态，不会重复请求发布源。发布前可添加 `release-notes/vX.Y.Z.txt` 编写该版本的简短说明；流水线将其写入 `version.json` 的 `summary` 字段（最多 160 字）。未提供时，使用上一标签之后最近 3 条非合并提交标题；旧发布源没有说明时，明确提示暂未提供，不编造更新内容。
